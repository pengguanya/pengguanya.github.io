...
**1. Use pinned memory for data loading.** Set `pin_memory=True` in the DataLoader and `dataloader_pin_memory=True` in TrainingArguments. This eliminates the pageable-to-pinned staging copy and allows truly asynchronous host-to-device transfers. On discrete GPUs, this significantly reduces `cudaMemcpyAsync` overhead during data loading. On unified memory architectures like the GB10, the benefit may be smaller since there is no physical bus transfer to optimize --- but the software path is still cleaner, and the recommendation ports directly to discrete GPU deployments.

**2. Use CUDA graphs or `torch.compile`.** The 273,186 individual `cudaLaunchKernel` calls (12.7% of API time) suggest kernel launch overhead is non-trivial. `torch.compile` fuses multiple operators into fewer kernels, reducing launch overhead and improving GPU occupancy. On Blackwell (sm_121), this also enables newer kernel optimizations.

**3. Adopt sequence packing.** The 2–10x step time variance is pure waste. Packing multiple short sequences into a single `max_length` window eliminates padding tokens and makes every batch the same computational cost. Libraries like `flash-attn` and HuggingFace's `DataCollatorWithFlattening` support this.
