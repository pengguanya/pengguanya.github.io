...
SAML authentication successful. Connecting to VPN...
  Server: <gateway-hostname>
  User: <username>
POST https://<gateway-hostname>/ssl-vpn/login.esp
...
```
HOST='https://<portal-hostname>/gateway:prelogin-cookie'
USER='<username>'
COOKIE='<long base64 string>'
OS='linux-64'
```

The cookie looked valid. But something caught my attention: the `HOST` URL contained `<portal-hostname>` --- that's the **portal**, not a **gateway**.
...
2. Portal discovers 12 available gateways and selects one (Basel: `<gateway-basel>`)
...
The script was authenticating against the **portal** (`<portal-hostname>`) and then relying on OpenConnect to negotiate with the gateway. This two-phase flow breaks because:
...
declare -A GATEWAYS=(
    ["Basel"]="<gateway-basel>"
    ["Mannheim"]="<gateway-mannheim>"
    ["Tokyo"]="<gateway-tokyo>"
    ["Santa_Clara"]="<gateway-santaclara>"
    # ... 12 gateways total
)
...
- **Recognizing the portal vs. gateway distinction**: The AI initially tried several approaches that worked around the stdin issue without questioning whether the cookie itself was valid for the gateway. I noticed the `<portal-hostname>` in the HOST field and asked "is a portal cookie even valid for gateway auth?" That question changed the direction of the investigation.
