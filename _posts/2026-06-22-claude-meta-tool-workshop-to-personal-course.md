...
![Four nested loops, each wrapping the one inside it. At the center, the Agent loop: reason, act, observe, repeat. Around it, the Verification loop: a separate checker grades the output and sends it back. Around that, the Event-driven loop: a trigger starts it. Outermost, the Hill-climbing loop: past runs rewrite the harness so it improves itself, and an arrow shows it reaching in to rewrite the inner loops. Each ring is annotated with how the workshop tool used that level.](/assets/img/2026-06-22-claude-meta-tool-workshop-to-personal-course/meta-tool-loops-stack.svg)
_The four loops nest: each one wraps the loop inside it, and the outermost reaches in to improve the rest. Each ring is annotated with how the workshop tool used that level._
